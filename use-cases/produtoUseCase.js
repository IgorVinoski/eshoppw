...
const updateProdutoDB = async (body) => {
    try {
        ...
        if (results.rowCount == 0) {
            throw `Nenhum produto encontrado com o código ${codigo} para ser alterado`;
        }
        const produto = results.rows[0];
        return new Produto(
            produto.codigo,
...
const deleteProdutoDB = async (codigo) => {
    try {
        ...
        if (results.rowCount == 0) {
            throw `Nenhum produto encontrado com o código ${codigo} para ser removido`;
        } else {
...
const getProdutoPorCodigoDB = async (codigo) => {
    try {
        ...
        if (results.rowCount == 0) {
            throw "Nenhum produto encontrado com o código: " + codigo;
        } else {
            const produto = results.rows[0];
            return new Produto(
                produto.codigo,
